...
### Added
- **Native Android rewrite (Kotlin + Jetpack Compose).** A new project rooted at the
  repository root. The web version is frozen and kept only as a reference.
- Design system foundation: theme colors, typography scale and shapes, to be used by
  every screen instead of inline styling.
- Navigation shell with library screens for songs, albums, artists and playlists.
...
## Web version history (Svelte + Capacitor)

> The entries below were never released. The web version was frozen when the native
> rewrite started; the last published APK is `0.7.0`.
