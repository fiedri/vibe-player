<script lang="ts">
  ...
  let isOpenedPlayer = $state(false);
  function handleOpenAndClosePlayer() {
    if (!player.currentSong) return;
    isOpenedPlayer = !isOpenedPlayer;
  }
</script>
...
{#if !isOpenedPlayer}
 ...
 <MarqueeText text={player.currentSong?.title} class="pr-14 text-white font-black"/>
    ...
    <div class="aspect-square h-auto p-5">
      <figure class="shadow-lg overflow-hidden">
        ...
        <figcaption class="mt-5 flex flex-col gap-3">
          ...
          <p class="text-muted-foreground text-xs truncate animate-seamless-marquee">
<MarqueeText text={player.currentSong?.album} class="pr-14 text-white font-medium"/>
          </p>
        </figcaption>
      </figure>
    </div>
